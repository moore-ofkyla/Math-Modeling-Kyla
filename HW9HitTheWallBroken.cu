...
void KeyPressed(unsigned char key, int x, int y)
{
	...
	//1)zero out COM 2) reset linear velocity to 0 3) new linear velocity 
	if(key=='!')
	{
		//Zero out COM and Linear Velocity 
		float4 pos, vel;
		Pause = 1;
		terminalPrint();
		pos = centerOfMass();
...
void terminalPrint()
{
	...
	*/
	...
	printf("\n !: Take your anger out on the balls(aka slam them against the wall)");//zeros COM and LV, and assigns new LV
