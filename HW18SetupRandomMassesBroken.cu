//nvcc HW18SetupRandomMassesBroken.cu -o bounce -lglut -lm -lGLU -lGL																													
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < (BodyRadius[i] + BodyRadius[j]))
			{
				...
				{
					r1=BodyRadius[i];//sets bodyradius[i] as the larger radius
					r2=BodyRadius[j];//sets bodyradius[j] as the smaller radius
				}
				else
				{
					r1=BodyRadius[j];//sets bodyradius[j] as the smaller radius
					r2=BodyRadius[i];//sets bodyradius[j] as the smaller radius
				}
				...
				temp = (d.w*d.w - r1*r1 + r2*r2)/(2*d.w);//this is our 'x'
				if(0.0<temp)
				{
