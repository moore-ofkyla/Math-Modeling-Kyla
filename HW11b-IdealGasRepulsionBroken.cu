//nvcc HW11b-IdealGasRepulsionBroken.cu -o bounce -lglut -lm -lGLU -lGL																													
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				if(inOut < 0.0) magnitude = cSphere*(intersectionArea/bodyVolume); // If inOut is negative the sphere are converging.
				//like our equation.(c/((PI/24)*(2D-d)(D+d)^2))*(PI/4(D^2-d^2))
