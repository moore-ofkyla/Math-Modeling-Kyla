//nvcc HW11a-QuadraticRepulsionBroken.cu -o bounce -lglut -lm -lGLU -lGL																													
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				intersectionArea = (PI/4.0)*(SphereDiameter*SphereDiameter - d.w*d.w);//quadratic force function 
				...
				if(inOut < 0.0) magnitude = intersectionArea*pSphere; // If inOut is negative the sphere are converging. 
				//pSphere*(PI/4.0)*(SphereDiameter*SphereDiameter - d.w*d.w).Where pSphere is like P in our equation

				else magnitude = pSphereReduction*intersectionArea; // If inOut is positive the sphere are diverging.
