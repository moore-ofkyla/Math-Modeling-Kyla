...
void KeyPressed(unsigned char key, int x, int y)
{
	...
	if(key == '1')
	{
		...
		//calls centerOfMass and linearVelocity function and both will return float value
		pos = centerOfMass();
		vel = linearVelocity();
...
void drawPicture()
{
	...
	glLineWidth(3.0);
	glColor3f(0.529f, 0.808f, 0.995f);//blue ish color
	glBegin(GL_LINE_LOOP);//used to draw square
	    glVertex3f(25.0f, 5.0f, -5.0f); //top left
	    glVertex3f(25.0f, 5.0f, 5.0f);  // top right 
	    glVertex3f(25.0f, -5.0f, 5.0f);   // bottom right
	    glVertex3f(25.0f, -5.0f, -5.0f);  // bottom left
	glEnd(); // Corrected this line
	glutSwapBuffers();
}
...
float4 centerOfMass()
{
	...
	//COM calculation=(Xi*Mi/Mt) where Mt is the total mass of the system
	for(int i=0; i< NUMBER_OF_BALLS; i++)
	{
		centerOfMass.x += Position[i].x*SphereMass;//I know sphere mass is 1 I just like knowing it's important. plus what if it's not 1
		centerOfMass.y += Position[i].y*SphereMass;
		centerOfMass.z += Position[i].z*SphereMass;
	}
	//Divide by mass total(# of balls * mass)
	
	centerOfMass.x /= (NUMBER_OF_BALLS*SphereMass);
	centerOfMass.y /= (NUMBER_OF_BALLS*SphereMass);
	centerOfMass.z /= (NUMBER_OF_BALLS*SphereMass);
...
float4 linearVelocity()
{
	...
	//LV= sum of velocities times the mass/ total mass
	for(int i=0; i< NUMBER_OF_BALLS; i++)
	{
		linearVelocity.x += Velocity[i].x*SphereMass;//I know spheremass is 1 right now, but maybe we want to change it at some point 
		linearVelocity.y += Velocity[i].y*SphereMass;
		linearVelocity.z += Velocity[i].z*SphereMass;
	}
	linearVelocity.x /= NUMBER_OF_BALLS*SphereMass;
	linearVelocity.y /= NUMBER_OF_BALLS*SphereMass;
	linearVelocity.z /= NUMBER_OF_BALLS*SphereMass;
