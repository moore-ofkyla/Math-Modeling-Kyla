//nvcc AsteriodsLocalDampingBrokenFinal.cu -o bounce -lglut -lm -lGLU -lGL																													
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d < SphereDiameter)
			{
				...
				inOut= dvx*(dx/d) + dvy*(dy/d) + dvz*(dz/d);//dot product. Helps determine how strong the asteroids are interacting along the collision line. 
				//dot product of relative velocity and unit vector(represents the direction)
				...
				magnitude= (SphereDiameter-d)*kSphereReduction*kSphere ;

				//magnitude= (SphereDiameter-d)*kSphereReduction*kSphere *(1.0+inOut*kSphereReduction);
				//magnitude with inOut, but I dont fully understand how it works

//ksphere=stifness of the repulsive force, inout= closer/farther, ksphereReduction=damping i think
//kSphere reduction damps it 
