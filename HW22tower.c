// nvcc HW22tower.c -o tower -lglut -lm -lGLU -lGL
//./tower
...
void set_initail_conditions()
{
	...
	CompressionStrength[1][4] = 10.0;//new
	TensionStrength[1][4] = 10.0;
	NaturalLength[1][4] = 2.0;
	
	CompressionStrength[2][4] = 10.0;//new
	TensionStrength[2][4] = 10.0;
	NaturalLength[2][4] = 2.0;
	
	CompressionStrength[3][4] = 10.0;//new
...
void draw_picture()
{
	...
	for(i = 0; i < N; i++)
	{
		...
		if(i == 4) glColor3d(1.0,0.412,0.706);//added color for fifth node.
		glPushMatrix();
	...
	glBegin(GL_LINE_STRIP);
		...
		glVertex3f(Px[4], Py[4], Pz[4]); 
	glEnd();
	glColor3d(Red[3][4],Green[3][4],Blue[3][4]);
	glBegin(GL_LINE_STRIP);
		glVertex3f(Px[3], Py[3], Pz[3]);   
		glVertex3f(Px[4], Py[4], Pz[4]); 
	glEnd();
