//nvcc HW7AsteriodsLocalDampingBrokenGood.cu -o bounce -lglut -lm -lGLU -lGL																													
...
void KeyPressed(unsigned char key, int x, int y)
{
	...
	float dy = 0.05f;
	float dz = 0.05f;
	if (key== 'r')//adding right 
	{
		glTranslatef(dx, 0.0, 0.0);
		drawPicture();
		terminalPrint();
	}
	if(key == 'l')//adding left
	{
		glTranslatef(-dx, 0.0, 0.0);
		drawPicture();
	}
...
void getForces()
{
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d < SphereDiameter)
			{
				dvx= Velocity[j].x-Velocity[i].x;//relative velocity.them minus me 
				dvy= Velocity[j].y-Velocity[i].y;
				dvz= Velocity[j].z-Velocity[i].z;

				inOut= dvx*(dx) + dvy*(dy) + dvz*(dz);/*dot product. Helps determine how strong the asteroids are interacting along the line of collision*/ 
				//dot product of relative velocity and direction
				//if this is positive, it means they are moving closer, if this is negative they are moving apart
				// ?????????????????
				...
				if (inOut<0)
				{/*if inOut<0 then the asteroids are getting closer meaning we want magnitude to just be (SphereDiamter-d)*kSphere,
				where spherediamter-d represents how much the asteroids overlap and kSphere is like the strength of a spring*/
				
				magnitude= (SphereDiameter-d)*kSphere ;
				}
				else
				{
				magnitude *= kSphereReduction ;
				//If inOut is positive it means they are moving apart, therefore we want to reduce the magnitude of the force
				}
//ksphere=stifness of the repulsive force, inout= closer/farther
//kSpherereduction damps it 
//SphereDiamter-d applies force depending on how much the balls overlap

				// Doling out the force in the proper perfortions using unit vectors.
				Force[i].x -= magnitude*(dx/d);//magnitude times direction in x
...
void terminalPrint()
{
	...
	*/
	...
	printf("\n r/l: Move right, move left");
	printf("\n u/d: Move up, move down");
